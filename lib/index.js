...
let jieba;
let PHRASES_DICT = require("../data/phrases-dict");
...
class NodePinyin extends Pinyin {
    ...
    convert(hans, options) {
        ...
        //如果词表里查到来这个词,则直接返回
        if (PHRASES_DICT.hasOwnProperty(hans)) {
            return PHRASES_DICT[hans]
        }
        options = assign({}, Pinyin.DEFAULT_OPTIONS, options);
        let phrases = options && options.segment ?
            segment(hans) :
            hans;
        let pys = [];
        let nohans = "";
        ...
        for (let i = 0, firstCharCode, words, l = phrases.length; i < l; i++) {
            ...
            console.log(words, firstCharCode);
